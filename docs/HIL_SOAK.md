...
| Layer | Where | Proves |
|---|---|---|
| Host sanity | `tools/ci_sanity.py` on every push | VERSION sync, one `.ino`, no AJ6 types, secrets not tracked |
| Compile | GitHub Actions `compile` job | Sketch builds for the Guition FQBN |
